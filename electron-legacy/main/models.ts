...
const MODELS_FILE = join(OCEAN_HOME, "models.json");
    ...
    for (const cp of cpModels) {
      ...
      if (cp.apiKey && cp.apiKey !== "no-key-required") {
        try {
          ...
          //      contract; the runtime spawn in `oceanos.ts` reads it
          ...
          // The gateway path in `oceanos.ts:startGateway` ingests ALL
