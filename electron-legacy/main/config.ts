...
import { OCEAN_HOME, expectedEnvKeyForModel } from "./installer";
import {
  escapeRegex,
...
// (OCEAN_HOME may not be assigned yet when this module first loads)
function desktopConfigFile(): string {
  return join(OCEAN_HOME, "desktop.json");
}
...
export function writeDesktopConfig(data: Record<string, unknown>): void {
  if (!existsSync(OCEAN_HOME)) {
    mkdirSync(OCEAN_HOME, { recursive: true });
  }
...
export function setModelConfig(
  ...
  // function early-returned, so users on a custom OCEAN_HOME where the
...
export function appendConfigFixLog(entry: ConfigFixLogEntry): void {
  try {
    const logDir = join(OCEAN_HOME, "logs");
 ...
 * just `{key, label}` were rejected at runtime ("Ocean is not
