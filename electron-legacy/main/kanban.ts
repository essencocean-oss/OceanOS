...
async function runKanban(
  ...
  // SSH tunnel mode: dispatch to the remote Ocean CLI over SSH.
...
export function unsupportedInRemote<T>(): KanbanResult<T> {
  return {
    ...
    error:
      "Kanban requires either a local Ocean install or SSH tunnel mode. " +
      "Plain remote (HTTP+API key) mode does not yet expose the kanban API. " +
      "Switch to SSH tunnel mode in Settings to use the board against a remote Ocean.",
  };
}
