...
import { getEnhancedPath, OCEAN_HOME } from "./installer";
...
const WS_URL_FILE = join(OCEAN_HOME, "claw3d-ws-url");
...
export function startAdapter(): boolean {
  ...
  proc.on("close", (code) => {
    if (code && code !== 0 && !adapterError) {
      adapterError = `Ocean adapter exited with code ${code}`;
    }
    adapterProcess = null;
    cleanupPid(ADAPTER_PID_FILE);
  });
...
export function startAll(): { success: boolean; error?: string } {
  ...
  if (!adapterOk) {
    return { success: false, error: "Failed to start Ocean adapter" };
  }
