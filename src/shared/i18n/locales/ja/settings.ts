export default {
  ...
  analytics: {
    ...
    hint: "プロジェクトの PostHog インスタンス（EU ホスト）に匿名・集計済みの利用状況データを送信することで OceanOS の改善に役立てます。いつでもオフにできます。",
    disclosure: {
  ...
  poolHint:
    "同じプロバイダの API キーを複数追加して自動ローテーション・負荷分散。OceanOS が順に使い回します。",
  add: "追加",
  remove: "削除",
  keyLabel: "キー",
  empty: "（空）",
  dataSection: "データ",
  dataHint:
    "OceanOS の設定、セッション、スキル、メモリのエクスポート・インポートを行います。",
  backingUp: "バックアップ中...",
  ...
  migrationDesc:
    "<code>{{path}}</code> に OpenClaw が見つかりました。設定・API キー・セッション・スキルを OceanOS に移行できます。",
  migrationDismiss: "再表示しない",
  migrating: "移行中...",
  migrateToOceanOS: "OceanOS に移行",
  skip: "スキップ",
  ...
  modeLocal: "ローカル",
  modeRemote: "リモート",
  modeLocalHint: "このデバイスにインストールされた OceanOS を使用",
  modeRemoteHint: "ネットワークまたはクラウド上の OceanOS API サーバに接続",
  remoteUrl: "リモート URL",
  remoteUrlHint:
    "OceanOS API サーバの URL（/health と /v1/chat/completions を公開している必要あり）",
  remoteApiKey: "API キー",
  remoteApiKeyHint:
