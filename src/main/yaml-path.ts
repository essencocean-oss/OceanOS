...
// a one-line lookup. Ocean config.yaml is plain key: value pairs nested by
// indentation — no anchors, no merge keys, no multi-line scalars in the
// fields we read. Edge cases we DO handle:
//
//  - 2-or-more-space indentation (Ocean always uses 2 today, but any
...
export function getYamlPath(content: string, dottedKey: string): string | null {
  ...
  for (const raw of lines) {
    ...
    if (!rawKey) continue;
    // Quoted keys aren't used in Ocean config but strip the wrapping just in
