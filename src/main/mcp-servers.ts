  ...
  return new Promise((resolve, reject) => {
    const child = execFile(
      ...
      oceanCliArgs(["mcp", ...args]),
      {
        ...
        env: {
          ...process.env,
          OCEAN_HOME: profilePaths(profile).home,
...
function unsupportedMcpApiMessage(feature: "catalog" | "install" | "test"): string {
  if (feature === "catalog") {
    return "MCP catalog is not available from this Ocean Agent gateway yet. Add a custom MCP server manually.";
  }
  if (feature === "install") {
    return "MCP catalog install is not available from this Ocean Agent gateway yet. Add a custom MCP server manually.";
  }
  return "MCP server testing is not available from this Ocean Agent gateway yet.";
}
