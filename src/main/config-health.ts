  ...
  if (!envKey && !topLevel && !nested) {
    ...
    if (configExists) {
      issues.push({
        ...
        severity: "warning",
        message:
          "No API_SERVER_KEY is set — chat will fail because the Ocean gateway requires auth.",
        detail:
          "API_SERVER_KEY is mandatory for Ocean API access. " +
...
/** Fields we compare across sibling ocean-homes. Each entry is
 *  `{ source, field, label }`:
...
/** Read the curated subset of fields from a sibling ocean-home. */
...
function checkSiblingHermesHomeDrift(profile?: string): ConfigHealthIssue[] {
  ...
  for (const sibling of siblings) {
    const siblingFields = readSiblingFields(sibling.oceanHome);
    ...
    for (const { field, label } of DRIFT_FIELDS) {
      ...
      // (assumption: Ocean One is the broken side, the user's
      // CLI on WSL is the working setup). If reverse direction is
      // ever needed, expose a second fixId.
      if (!winValue && wslValue) {
        issues.push({
          code: "SIBLING_OCEAN_HOME_DRIFT",
          severity: "warning",
          message: `${label} is set on WSL (${sibling.distro}) but not on the Windows side that Ocean One reads.`,
          detail:
            `WSL value (${where}): ${wslMasked}\n` +
            `Windows value: (not set)\n\n` +
            `Ocean One reads only ${current.envFile.replace(/\\\.env$/, "")} — your CLI on WSL works, the desktop doesn't, because the value never made it across. Auto-fix copies the WSL value into the Windows-side file.`,
          locations: [current.configFile, current.envFile, sibling.oceanHome],
          autoFixable: true,
        ...
        issues.push({
          ...
          detail:
            ...
            `Ocean One reads the Windows side, so this isn't blocking the desktop. Just a heads-up that your CLI on WSL is missing this value if you also use it there.`,
          locations: [current.envFile, sibling.oceanHome],
          autoFixable: false,
          context: {
            field,
            distro: sibling.distro,
        ...
        issues.push({
          ...
          detail:
            `Windows value: ${winMasked}\n` +
            `WSL value (${where}): ${wslMasked}\n\n` +
            `Ocean One reads only the Windows side. If these were supposed to be the same, copy whichever value is current to the other side. If they're intentionally different, this notice is informational.`,
          locations: [current.envFile, sibling.oceanHome],
          autoFixable: false,
          context: {
            field,
...
function fixSiblingHermesHomeDrift(
  ...
  try {
    ...
    appendConfigFixLog({
      ts: Date.now(),
      issueCode: "SIBLING_OCEAN_HOME_DRIFT",
      action: "autofix",
...
export function configFixLogPath(): string {
  return join(OCEAN_HOME, "logs", "config-fixes.log");
}
