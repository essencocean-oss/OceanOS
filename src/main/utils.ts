...
export function profileHome(profile?: unknown): string {
  ...
  return normalized ? join(OCEAN_HOME, "profiles", normalized) : OCEAN_HOME;
}
 ...
 * process is ours.
 *
 * Important for the WSL coexistence case: when OCEAN_HOME points into WSL
...
export function getActiveProfileNameSync(): string {
  try {
    const activeFile = join(OCEAN_HOME, "active_profile");
