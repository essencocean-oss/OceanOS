...
function isDashboardReady(baseUrl: string, token: string): Promise<boolean> {
  return new Promise((resolve) => {
    const req = http.request(
      ...
      {
        method: "GET",
        headers: { "X-Ocean-Session-Token": token },
        timeout: 1500,
      },
      (res) => {
...
async function waitForDashboardReady(
  ...
  throw new Error("Ocean dashboard gateway did not become ready");
}
...
class TuiGatewayClient {
  ...
  async request<T>(
    ...
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
      throw new Error("Ocean dashboard gateway stream is not connected");
    }
    ...
    return new Promise<T>((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(new Error(`Ocean gateway request timed out: ${method}`));
      }, timeoutMs);
      timer.unref?.();
      this.pending.set(id, {
        reject,
  ...
  stop(): void {
    ...
    this.rejectPending(new Error("Ocean dashboard gateway stream stopped"));
    this.reset();
  }
  ...
  private async startDashboardBackend(): Promise<void> {
    ...
    const exitBeforeReady = new Promise<never>((_resolve, reject) => {
      ...
      proc.once("exit", (code, signal) => {
        reject(
          new Error(
            `Ocean dashboard gateway exited before ready (${signal || code})`,
          ),
        );
      });
    });
    ...
    proc.once("exit", (code, signal) => {
      const error = new Error(
        `Ocean dashboard gateway exited (${signal || code})`,
      );
  ...
  private connectWebSocket(url: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.ws = ws;
      const timer = setTimeout(() => {
        reject(new Error("Ocean dashboard gateway WebSocket timed out"));
        ws.close();
      }, 15_000);
      timer.unref?.();
      ...
      ws.on("close", () => {
        ...
        const error = new Error("Ocean dashboard gateway WebSocket closed");
        this.rejectPending(error);
        this.reset();
      });
    });
  ...
  private handleFrame(raw: string): void {
    ...
    if (frame.id != null) {
      ...
      if (frame.error) {
        pending.reject(new Error(frame.error.message || "Ocean RPC failed"));
      } else {
...
function waitForGatewayEvent(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      cleanup();
      reject(new Error("Timed out waiting for Ocean gateway readiness"));
    }, timeoutMs);
    timer.unref?.();
...
function sendMessageViaApi(
  ...
  // Session id: always send via `X-Ocean-Session-Id` so the gateway
  ...
  // state.db / logs.
  //
  // Gate on auth: the gateway rejects `X-Ocean-Session-Id` with 403
  ...
  if (sessionId) {
    headers["X-Ocean-Session-Id"] = sessionId;
  }
  ...
  req.on("timeout", () => {
    finish(
      "API request timed out. Check the SSH tunnel and remote Ocean gateway.",
    );
    req.destroy();
  });
...
function sendMessageViaRuns(
  ...
  function handleRunEvent(raw: Record<string, unknown>): void {
    ...
    if (eventName === "run.cancelled") {
      finish(hasContent ? undefined : "Ocean run was cancelled.");
      return;
    }
...
async function sendMessageViaTuiGateway(
  ...
  function startApiFallback(reason: string): void {
    ...
    client.stop();
    console.warn(
      "[chat] Ocean gateway stream failed before output; falling back to API stream:",
      reason,
    );
  ...
  cleanup = client.onEvent((event) => {
    ...
    if (event.type === "error") {
      ...
      const error =
        typeof event.payload?.message === "string"
          ...
          : "Ocean gateway stream reported an error.";
      if (!hasGatewayOutput) {
    ...
    if (event.type === "approval.request") {
      // Match the existing local chat posture: Ocean One does not expose a
    ...
    if (event.type === "clarify.request") {
      ...
      if (!requestId) {
        ...
        void client
          ...
          .catch(() => undefined);
        finish(
          "Ocean requested clarify input, but the gateway provided no request_id to answer.",
        );
        return;
      }
    ...
    if (event.type === "sudo.request" || event.type === "secret.request") {
      ...
      void client
        ...
        .catch(() => undefined);
      finish(
        `Ocean requested ${event.type.replace(".request", "")} input, but Ocean One does not yet expose that gateway dialog.`,
      );
    }
  });
  ...
  try {
    ...
    if (!activeSessionId) {
      throw new Error("Ocean gateway did not return a session id");
    }
...
function sendMessageViaCli(
  ...
  proc.on("close", (code) => {
    if (code === 0 || hasOutput) {
      ...
      cb.onError(
        detail
          ? `Ocean exited with code ${code}: ${detail}`
          : `Ocean exited with code ${code}. Check your model configuration and API key.`,
      );
    }
  });
...
async function sendMessageViaBestApi(
  ...
  if (
    ...
    try {
      ...
      console.warn(
        "[chat] Ocean gateway stream unavailable; falling back to API stream:",
        error instanceof Error ? error.message : String(error),
      );
    }
  }
...
function buildGatewayEnv(profile?: string): Record<string, string> {
  ...
  // `Authorization: Bearer <key>` + `X-Ocean-Session-Id` for users
...
export function startGatewayDetailed(profile?: string): GatewayStartResult {
  ...
  if (isRemoteMode()) {
    const error =
      "The local gateway can only be started in local mode. Switch to local mode, or start the gateway on the remote Ocean host.";
    console.warn(
  ...
  // OCEAN_HOME at the profile's dir internally; the shared repo/venv stay
