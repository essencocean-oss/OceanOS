...
// Resolve the Ocean data directory. Precedence:
...
// A Ocean home the user explicitly pointed the app at via the "use an
// existing installation" flow (issue #272). Persisted in the desktop's own
// userData dir — outside any Ocean home — so it can be read here, before
// OCEAN_HOME is resolved. Strictly additive: with no override file the
// behaviour is identical to before.
function oceanHomeOverrideFile(): string {
...
function readHermesHomeOverride(): string {
  try {
    ...
    const p =
      typeof parsed.oceanHome === "string" ? parsed.oceanHome.trim() : "";
...
export function setHermesHomeOverride(home: string): void {
  try {
    ...
    writeFileSync(
      file,
      JSON.stringify({ oceanHome: home.trim() }, null, 2),
      "utf-8",
    );
  } catch {
...
export function getEnhancedPath(): string {
  ...
  const extra = (
    IS_WINDOWS
      ? [
          // Bundled by install.ps1 inside OCEAN_HOME — these matter when the
          // user's system PATH doesn't include git or node yet.
          join(OCEAN_HOME, "git", "bin"),
...
export function checkInstallStatus(): InstallStatus {
  ...
  // (including credential-pool entries) can be configured through Ocean
...
// ocean-desktop itself creates ~/.openclaw/claw3d/ as a stub when preparing
...
export async function runInstall(
  ...
  emit("Running official Ocean install script...\n");
...
export function readLogs(
  ...
  const logsDir = join(OCEAN_HOME, "logs");
