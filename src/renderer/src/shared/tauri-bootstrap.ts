// Tauri bootstrap shim: exposes window.hermesAPI for the Electron-era React UI.
// Loaded before main.tsx so the existing code keeps working.
const { invoke } = window.__TAURI__?.core || {};
const { open } = window.__TAURI__?.shell || {};
const fs = window.__TAURI__?.fs || {};
const notifs = window.__TAURI__?.notification || {};

function toPromise(fn) {
  return (...args) => (fn ? fn(...args) : Promise.resolve(undefined));
}
...
window.hermesAPI = {
  ...
  setLocale: toPromise((locale) => Promise.resolve(locale)),
  ...
  getConfigFixLog: toPromise(() => Promise.resolve([])),
  ...
  setModelConfig: toPromise((_provider, _model, _baseUrl, _profile) =>
